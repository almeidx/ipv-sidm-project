...
export async function makeApiRequest<Result>(endpoint: string, init?: MakeApiRequestOptions): Promise<MakeApiRequestResult<Result>>;
export async function makeApiRequest<Result>(endpoint: string, {query, failMessage = "Unknown error when getting data", ...init}: MakeApiRequestOptions = {}): Promise<MakeApiRequestResult<Result> | null> {
  try {
